...
const {Adapter, TextMessage, EnterMessage, LeaveMessage, TopicMessage, CatchAllMessage} = require.main.require(Path.resolve(__dirname, "../../index.js"))
...
class SlackBot extends Adapter {
  ...
  eventHandler(event) {
    ...
    if (event.type === "message") {
      ...
      const users = (() => {
        const result = []
        for (let user_id of Array.from((event.users || [event.user.id]))) {
          if (this.robot.brain.data.users[user_id] != null) {
            result.push(this.robot.brain.data.users[user_id])
          }
        }
        return result
      })()
